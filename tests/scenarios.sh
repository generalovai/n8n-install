...
run() { printf "%-52s" "$1"; shift; printf "$1" | bash "${INSTALL_SH:-/work/install.sh}" >/tmp/r.log 2>&1; c=$?; e=$(grep -c 'ОШИБКА' /tmp/r.log); [ $c -eq 0 ] && [ $e -eq 0 ] && echo "OK" || { echo "ПРОВАЛ код=$c ошибок=$e"; tail -5 /tmp/r.log; }; }
